...
internal sealed class TenantTransactionAttemptState(ITenantAttemptOrderObserver observer)
{
    ...
    public void RecordCommand(DbCommand command, CommandEventData eventData, bool write)
    {
        ...
        Record(source.Contains("Sql", StringComparison.Ordinal)
            ? TenantAttemptCommandCategory.EfRawSql
            : write || StringComparer.Ordinal.Equals(source, "SaveChanges")
                ? TenantAttemptCommandCategory.EfWrite
...
internal sealed class TenantTransactionExecutor(ApplicationDbContext database, ITenantContextAccessor tenant,
    TenantTransactionAttemptState attempt, TenantPostCommitActions postCommit) : ITenantTransactionExecutor
{
    ...
    public async Task<T> ExecuteOnceAsync<T>(Func<CancellationToken, Task<T>> work,
