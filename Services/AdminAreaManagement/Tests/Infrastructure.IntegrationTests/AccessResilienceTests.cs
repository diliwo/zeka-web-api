...
public sealed class AccessResilienceTests
{
    ...
    public async Task Target_membership_adapter_also_preserves_authentication_failure(int status)
    {
        ...
        if (status == 403) Assert.False(await adapter.IsActiveAsync(Guid.NewGuid(), Guid.NewGuid(), default));
        else
        {
            var exception = await Assert.ThrowsAsync<TenantAccessException>(() => adapter.IsActiveAsync(Guid.NewGuid(), Guid.NewGuid(), default));
